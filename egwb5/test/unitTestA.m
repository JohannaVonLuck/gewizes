...
void egwRBTreePrint(egwRedBlackTree* tree_in, egwRedBlackTreeNode* node_in) {
    if(node_in) {
        egwRBTreePrint(tree_in, node_in->left);
        printf("%p:%s\r\n", node_in, (char*)((EGWuintptr)node_in + (EGWuintptr)sizeof(egwRedBlackTreeNode)));
        egwRBTreePrint(tree_in, node_in->right);
    }
}
...
- (void)applicationDidFinishLaunching:(UIApplication*)application { // on main thread
    ...
    {   [application setIdleTimerDisabled:YES];
        ...
        [egwAISndCntx setSystemVolume:10];
    ...
    {   egwMesh* testMesh = nil;
        if(testMesh = [[egwMesh alloc] initBoxWithIdentity:@"testBox" boxWidth:1.0f boxHeight:1.0f boxDepth:1.0f geometryStorage:EGW_GEOMETRY_STRG_VBOSTATIC lightStack:nil materialStack:nil textureStack:[[[egwTextureStack alloc] initWithTextures:(id<egwPTexture>)[egwSIAsstMngr retrieveAsset:@"testBoxTexture"],0] autorelease]]) {
    ...
    {   egwPSParticleDynamics pDyn; memset((void*)&pDyn, 0, sizeof(egwPSParticleDynamics));
        ...
        if(testSystem = [[egwParticleSystem alloc] initWithIdentity:@"testSystem" particleDynamics:&pDyn systemDynamics:&sDyn systemBounding:nil geometryStorage:EGW_GEOMETRY_STRG_VBOSTATIC lightStack:nil textureStack:[[[egwTextureStack alloc] initWithTextures:tex,0] autorelease]]) {
